...
Reference Title: Spring AI + Spring Security: Authentication & Authorization
...
**The damage:**
- 📈 **$47,283** in unexpected OpenAI API costs
- 🔓 **2,847** customer conversations exposed
- 📉 **34%** drop in trust score
- ⏰ **3 weeks** of reputation repair
- 💼 **2 engineers** fired
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) {
        return http
            .authorizeHttpRequests(auth -> auth
                .requestMatchers("/api/ai/**").authenticated()  // ← This line
...
```
User → Login → Access Resource → View Data
        ↑
    Single check
```
...
```
User → Login → AI Endpoint → LLM → Response → Filter → User
        ↑        ↑            ↑      ↑        ↑
...
Direct Costs
├─ Unauthorized API usage:        $15,000 - $100,000
├─ Emergency fixes:                $5,000 - $25,000
├─ Legal/compliance:              $10,000 - $500,000
└─ Infrastructure hardening:       $8,000 - $50,000

Indirect Costs
├─ Customer trust loss:           20-40% churn
├─ Reputation damage:              6-18 months recovery
├─ Lost productivity:              2-4 weeks
...
☐ Multi-factor auth for admin endpoints
...
|----------|---------------|------------|----------|
| **No Auth** | ❌ None | Simple | Never use in production |
| **Basic Auth** | ⭐ Low | Simple | Internal tools only |
| **API Keys** | ⭐⭐ Medium | Medium | Service-to-service |
| **OAuth2 + JWT** | ⭐⭐⭐⭐ High | Medium | User-facing apps |
| **mTLS** | ⭐⭐⭐⭐⭐ Very High | Complex | Enterprise B2B |
...
**Total: 1 week from unprotected to production-ready** ✅
